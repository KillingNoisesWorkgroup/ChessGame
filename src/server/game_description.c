...
uint8_t get_fig_type(desk_t *desk, uint8_t i, uint8_t j){
	return desk->cells[i+j*8].type;
}

uint8_t get_fig_color(desk_t *desk, uint8_t i, uint8_t j){
	return desk->cells[i+j*8].color;
}
